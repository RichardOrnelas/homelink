...
resource "aws_security_group" "alb_public" {
  ...
  tags = {
    Name    = "${var.project}-${terraform.workspace}-alb-public"
  }
}
...
resource "aws_security_group" "rds" {
  ...
  tags = {
    Name        = "${var.project}-${terraform.workspace}-rds-access"
  }
}
...
resource "aws_s3_bucket" "bucket" {
  bucket = "tf-${var.project}-${var.environment}"
}
...
resource "aws_sqs_queue" "queue" {
  ...
  name                      = "${var.project}_${var.environment}_${element(var.queues, count.index)}"
  delay_seconds             = 0
...
resource "aws_sqs_queue" "queue_dead" {
  count                     = length(var.queues)
  name                      = "${var.project}_${var.environment}_${element(var.queues, count.index)}_dead"
  delay_seconds             = 0
  max_message_size          = 262144
...
resource "aws_iam_role" "platform_service" {
  name = "tf-${var.project}-${var.environment}"
